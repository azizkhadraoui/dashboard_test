...
const TabsOpen = () => {
  ...
  const handleSubmit = async () => {
    ...
    for (const user of users) {
      const userValue =
        updatedData.find((item) => item.id === user.name)?.value || 0;
      await addSeatToFlight(
        newval[4],
        newval[3],
        newval[0],
        userValue,
  ...
  const handleTabChange = (event, newValue) => {
    ...
    setCounters([]);
    setNewval([
      flights[newValue.toString()].empty_seats,
  ...
  async function addSeatToFlight(
    flightDate,
    flightType,
    emptySeats,
    seatNumber,
          ...
          {flights.length > 0 && (
            <TabContext value={activeTab}>
              ...
              <TabPanel value={activeTab}>
                <Box
                  sx={{ padding: "20px", backgroundColor: colors.background }}
                >
                  <Box
                    sx={{
                      display: "flex",
                  ...
                  >
                    <Grid container spacing={2}>
                      <Grid item xs={6}>
                        <TableContainer
                          component={Paper}
                          sx={{ height: 360, overflowY: "scroll" }}
                        >
                              ...
                              {users?.map((user) => (
                                <TableRow key={user.id}>
                                  ...
                                  >
                                    <Button
                                      sx={{ color: "red" }}
                                      onClick={() => decrementCounter(user.id)}
                                    >
                                      -
                                    </Button>
                                    {counters[user.id] || 0}
